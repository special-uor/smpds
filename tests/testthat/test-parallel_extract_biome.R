test_that("parallel_extract_biome works", {
  ...
  expect_equal(parallel_extract_biome(test_data_t1, cpus = 1), expected_t1)
  ...
  expect_equal(parallel_extract_biome(test_data_t2, cpus = 1), expected_t2)

  # Invalid data frame as input, missing longitude
  test_data_t3 <- tibble::tibble(entity_name = "University of Reading",
                                 latitude = 51.44140,
                                 longitude = NA)
  expect_equal(parallel_extract_biome(test_data_t3, cpus = 1), test_data_t3)

  # Invalid input format, list
  expect_error(parallel_extract_biome(
    list(entity_name = "University of Reading",
         latitude = 51.44140,
         ongitude = -0.9418)
    ))
})
